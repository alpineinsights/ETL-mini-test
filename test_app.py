...
def initialize_clients() -> bool:
    ...
    try:
        ...
        qdrant_client = initialize_qdrant()  # Use the function from init_utils
        ...
        try:
            qdrant_adapter = QdrantAdapter(
                ...
                collection_name=st.session_state.collection_name,  # Use from session state
        ...
        cleanup_clients()  # Clean up on failure
        return False
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_file:
            ...
            try:
                ...
                # Handle the result based on LlamaParse's response format
                if isinstance(result, list):
                    # If multiple documents returned, concatenate their text
...
with st.sidebar:
    ...
    if st.button("Delete Collection"):
        try:
            st.session_state.clients['qdrant_adapter'].delete_collection()
    ...
    if st.button("Create Collection"):
        try:
            st.session_state.clients['qdrant_adapter']._ensure_collection_exists()
    ...
    try:
        if 'qdrant_adapter' in st.session_state.clients:
            info = st.session_state.clients['qdrant_adapter'].get_collection_info()
            st.write(info)
        else:
            st.error("Qdrant adapter not initialized")
    except Exception as e:
